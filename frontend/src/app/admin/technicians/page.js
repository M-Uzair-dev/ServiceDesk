...
export default function TechniciansPage() {
  ...
  const load = (p = 1) => {
    setLoading(true)
    apiFetch(`/admin/technicians?page=${p}&limit=20`)
    ...
    <div className="p-8 pt-14 md:pt-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{data.items.length > 0 ? `${data.items.length} shown` : ''}</p>
        </div>
        <Link
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <Link href="/admin/technicians/new" className="text-sm text-gray-900 font-medium mt-2 inline-block hover:underline">
          ...
          <table className="w-full min-w-[700px] text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-5 py-3 text-left text-xs font-medium text-gray-500 min-w-[120px]">Skills</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-gray-500 min-w-[60px]">Exp.</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-gray-500 min-w-[80px]">Status</th>
            ...
            <tbody>
              {data.items.map((tech) => (
                <tr key={tech.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3 font-medium text-gray-900">{tech.name}</td>
                  <td className="px-5 py-3 text-gray-600">{tech.email}</td>
                  <td className="px-5 py-3 text-gray-500">{tech.phoneNumber || '—'}</td>
                  <td className="px-5 py-3 text-gray-500 max-w-32 truncate">
                    {Array.isArray(tech.skills) ? tech.skills.join(', ') : (tech.skills || '—')}
                  </td>
                  <td className="px-5 py-3 text-gray-500">
                    {tech.experienceYears != null ? `${tech.experienceYears}y` : '—'}
                  </td>
                  <td className="px-5 py-3">
                    <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                      tech.status === 'ACTIVE' || !tech.status ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'
                    }`}>
                      {tech.status || 'ACTIVE'}
                    </span>
                  </td>
